...
\begin{description}
	...
	\end{itemize}
	\item \code{WAITING_RESPONSE}: rappresenta lo stato di attesa della risposta da parte del CS. Siccome 
	\item \code{WAITING_CONFIRM}:
	\item \code{PARKING}:
	\item \code{GO_TO_RECHARGE}:
	\item \code{WAITING_EVSE}:
	\item \code{CHARGING}:
\end{description}
